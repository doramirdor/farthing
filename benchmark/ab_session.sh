...
call(){ local cont="$2"   # "" for turn 1, "--continue" after
  if [ "$ARM" = headroom ]; then ANTHROPIC_BASE_URL="$PROXY" claude -p "$1" $cont --allowedTools "$TOOLS" --output-format json 2>/dev/null
  else claude -p "$1" $cont --allowedTools "$TOOLS" --output-format json 2>/dev/null; fi; }
