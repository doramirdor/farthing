...
4. **farthing terse** is the only saver (−5%), and small — accuracy held at 100%.
5. No single-lane compressor delivers real savings here; the money is in **cache
   dynamics** (Nadir's model-routing axis, or farthing's multi-turn coordination),
...
layer** via `--append-system-prompt`. Faithful to the output lane; farthing's
...
| farthing audit ($15,937 bill, 0.3% avoidable) | **raw** — [`data/scrooge_audit_summary.json`](../data/scrooge_audit_summary.json) |
| farthing terse-output A/B (+0.2% output) | **aggregate only** — private corpus, raw excluded from public repo |
...
## 1. headroom on Q&A: net +17% **[superseded harness — see retractions]**
...
## 2. ~~headroom on coding tasks: 0% — it compressed nothing~~ **[SUPERSEDED]**

> **Superseded.** This section described headroom's **default `cache` mode**, which
> freezes the prefix and compresses nothing. In its **`token` mode** (the compressing
> default users actually run) it *does* compress real code — measured at **15–41%,
> up to 86k tokens removed** — and the bill still rose **45–282%**, because the
> rewrite busts the cache. See [`results/express_real_project.md`](../results/express_real_project.md).
> The claim "headroom is cache-safe by design" below is **wrong** and kept only as a
> record of what we believed at the time.
...
## 2b. farthing terse-output on the same 20 prompts: +0.2% output = null **[superseded harness]**

Ran the identical 20 prompts, baseline vs `claude -p --append-system-prompt
"<farthing terse-output rules>"`. Full table:
...
- **2/20 degenerate** (model answered nothing under terse pressure) — a quality
  regression, exactly what farthing's own canary gate exists to catch.

**Same wall as headroom.** farthing's layers (terse reinforcement per turn,
...
On single-shot `claude -p`: **headroom = 0%** (prefix_frozen), **farthing terse =
...
dominated by cache variance. This *is* farthing's thesis: microbenchmarks mislead;
...
## 4. farthing audit of the real bill
...
transcribed from the live run. The Q&A A/B, headroom ledger, and farthing audit are
